...
import { Calendar as CalendarIcon, BookOpen, Clock, Calendar, Check, Star, Brain, Target } from 'lucide-react';
import { Calendar } from '@/components/ui/calendar';
import { useVoiceAnnouncerContext } from '@/components/dashboard/student/voice/VoiceAnnouncer';
...
const StudyPlanPage = () => {
  ...
  // Load sample study plan data or use defaults
  useEffect(() => {
  ...
  // Announce study plan when component mounts
  useEffect(() => {
    if (selectedDateEvents.length > 0) {
      const totalTime = selectedDateEvents.reduce((acc, curr) => acc + curr.duration, 0);
      const subjects = [...new Set(selectedDateEvents.map(e => e.subject))].join(", ");
      
      voiceAnnouncer.speak(`Your study plan for today includes ${selectedDateEvents.length} sessions on ${subjects}, with a total duration of ${totalTime} minutes. I've color-coded the high priority tasks for you.`);
    }
  }, []);
