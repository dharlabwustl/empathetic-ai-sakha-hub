...
const PracticeExamFeature = ({ displayCount = 3 }: { displayCount?: number }) => {
  ...
  // Helper function for getting current card
  const getCurrentCard = () => {
    if (!examInProgress) return null;
    ...
    <Card className="shadow-md border border-gray-200 dark:border-gray-800">
      ...
      <AnimatePresence>
        {showExamEnvironment && examInProgress && (
          <ExamEnvironment
            exam={examInProgress}
            currentQuestion={getCurrentCard()}
            currentQuestionIndex={currentQuestionIndex}
            questions={sampleQuestions[examInProgress.id] || []}
      ...
      <AnimatePresence>
        {showExamReview && reviewingExam && (
          <ExamReview
            exam={reviewingExam}
            reviewData={examReviewData[reviewingExam.id]}
            closeExamReview={closeExamReview}
            handleToast={(title, description) => toast({ title, description })}
          />
        )}
      </AnimatePresence>
