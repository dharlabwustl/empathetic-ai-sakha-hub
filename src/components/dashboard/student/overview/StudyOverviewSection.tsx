...
interface StudyOverviewSectionProps {
  ...
  suggestions: string[];
  pageType: 'concepts' | 'flashcards' | 'practice-exam';
}

const StudyOverviewSection: React.FC<StudyOverviewSectionProps> = ({
  title,
  subjects,
  totalStudyTime,
  overallProgress,
  ...
  const getPageIcon = () => {
    switch (pageType) {
      case 'concepts': return <BookOpen className="h-5 w-5 text-indigo-600" />;
      case 'flashcards': return <Zap className="h-5 w-5 text-purple-600" />;
  ...
  const getPageColor = () => {
    switch (pageType) {
      case 'concepts': return 'from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20 border-indigo-200 dark:border-indigo-800';
      case 'flashcards': return 'from-purple-50 to-violet-50 dark:from-purple-900/20 dark:to-violet-900/20 border-purple-200 dark:border-purple-800';
      case 'practice-exam': return 'from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-green-200 dark:border-green-800';
      default: return 'from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-blue-200 dark:border-blue-800';
    }
  };
  ...
  return (
    <div className="space-y-6 mb-8">
      ...
      {/* Progress Overview */}
      <Card className={`bg-gradient-to-r ${getPageColor()}`}>
        <CardHeader>
          ...
          <div className="space-y-4">
            {subjects.map((subject, index) => (
              <motion.div
                ...
                className="space-y-3 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm"
              >
      ...
      <Card className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 border-indigo-200 dark:border-indigo-800">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Smart AI Suggestions for {pageType === 'concepts' ? 'Concept Learning' : pageType === 'flashcards' ? 'Flashcard Review' : 'Practice Exams'}
          </CardTitle>
        </CardHeader>
        <CardContent>
