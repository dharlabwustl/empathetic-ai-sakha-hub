  ...
  React.useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (dashboardStats) {
          // Update the stats with dynamic data from the backend
          setStats([
            {
              icon: <Users className="h-8 w-8 text-blue-500" />,
        ...
        console.error('Error fetching dynamic KPI stats:', error);
        // Keep fallback values if backend data is unavailable
      }
    };
