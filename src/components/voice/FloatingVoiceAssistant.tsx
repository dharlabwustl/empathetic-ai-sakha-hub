...
import VoiceAccentSelector from "@/components/voice/VoiceAccentSelector";
import { getPreferredAccent } from "@/components/dashboard/student/voice/voiceUtils";
import type { SupportedLanguage } from "@/components/dashboard/student/voice/voiceUtils";
...
const FloatingVoiceAssistant: React.FC<FloatingVoiceAssistantProps> = ({ 
  ...
  const [voiceLanguage, setVoiceLanguage] = useState<SupportedLanguage>(language as SupportedLanguage);
  ...
  const speakMessage = (message: string) => {
    if (!isMuted && 'speechSynthesis' in window) {
      ...
      // Filter by gender preference and language
      const preferredVoices = voices.filter(voice => {
        const matchesLanguage = voice.lang.startsWith(voiceLanguage);
  ...
  const handleAccentChange = (accent: string) => {
    setVoiceLanguage(accent as SupportedLanguage);
    ...
    toast({
      title: "Voice accent changed",
      description: `Voice accent changed to ${accentNames[accent] || accent}`,
    });
  };
